...
int main() {
	...
	XTPG_mWriteSlaveReg0(XPAR_AXI_TPG_0_BASEADDR, 0, (5<<6)|(Reg32Value&0xFFFFFE3F));
...
void camera_loop(camera_config_t *config) {
	...
	for (j = 0; 1; j++) {
		for (i = 0; i < 1920*1080/2; i++) {
			pMM2S_Mem[i] = pS2MM_Mem[1920*1080-2*i-1];
			pMM2S_Mem[i+1920*1080/2] = pS2MM_Mem[1920*1080-2*i-2];
		}
	}
